...
namespace CreativeGurus.Weather.Wunderground
{
	internal static class UrlBuilder
	{
		internal static string BuildUrl(string baseUrl, string apiKey, Feature feature, QueryType queryType, QueryOptions options)
		{
			...
			if (options.UsePWS.HasValue) { sb.AppendFormat("/pws:{0}", options.UsePWS.Value); } // Use Personal weather station
			if (options.UseBestFct != null) { sb.AppendFormat("/bestfct:{0}", options.UseBestFct); } // Use weather underground best forecast for forecast
